...
public class QueryIdCachingProxyHandler extends ProxyHandler {
  ...
  public static final String PRESTO_SOURCE_HEADER = "X-Presto-Source";
  public static final String TRINO_USER_HEADER = "X-Trino-User";
  ...
  private QueryHistoryManager.QueryDetail getQueryDetailsFromRequest(HttpServletRequest request)
    ...
    queryDetail.setUser(getUserFromRequest(request));
    queryDetail.setSource(getSourceFromRequest(request));
    String queryText = CharStreams.toString(request.getReader());
  ...
  String getUserFromRequest(HttpServletRequest request) throws IOException {
    String prestoHeader = request.getHeader(PRESTO_USER_HEADER);
    String trinoHeader = request.getHeader(TRINO_USER_HEADER);
    if (prestoHeader == null && trinoHeader == null) {
      throw new IOException("User Header missing");
    } else if (prestoHeader == null) {
      return trinoHeader;
    } else {
      return prestoHeader;
    }
  ...
  String getSourceFromRequest(HttpServletRequest request) {
    String prestoHeader = request.getHeader(PRESTO_SOURCE_HEADER);
    String trinoHeader = request.getHeader(TRINO_SOURCE_HEADER);
    if (prestoHeader == null && trinoHeader == null) {
      log.warn("Source Header missing");
      return prestoHeader;
    } else if (prestoHeader == null) {
      return trinoHeader;
    } else {
